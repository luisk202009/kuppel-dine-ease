...
-- Using only existing roles: admin, cashier, waiter
...
-- 3. Add RLS INSERT policy for tables (admin can create)
...
-- 4. Expand RLS policies for products - allow admins to manage products
...
-- 5. Expand RLS policies for categories - allow admins to manage categories  
...
-- 8. Update user luis.moreno@kuppel.co to admin role
UPDATE public.users SET role = 'admin' WHERE email = 'luis.moreno@kuppel.co';

-- 9. Create demo company, branch and associate user
...
-- Create demo categories for the company
...
-- Create demo tables for the branch
