...
serve(async (req) => {
  ...
  try {
    ...
    const { invitationId, email, companyName, role, inviterName }: InvitationRequest = await req.json();

    console.log('Processing invitation:', { invitationId, email, companyName, role });
    ...
    if (resendApiKey) {
      const emailResponse = await fetch('https://api.resend.com/emails', {
        ...
        body: JSON.stringify({
          from: 'Kuppel <noreply@kuppelapp.com>',
          to: [email],
    ...
    return new Response(
      JSON.stringify({ 
        ...
        inviteUrl: resendApiKey ? undefined : inviteUrl // Return URL only in dev mode
      }),
      { 
